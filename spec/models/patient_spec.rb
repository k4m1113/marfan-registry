...
describe Patient do
  ...
  it { should_not have_valid(:month_of_birth).when("Dec", "12", nil, "", -1, 13) }

  it { should have_valid(:day_of_birth).when(1, 31, 28) }
  it { should_not have_valid(:day_of_birth).when("Mon", "12", nil, "", -1, 32) }

  it { should have_valid(:year_of_birth).when(1992, 2016) }
  it { should_not have_valid(:year_of_birth).when(1899, 2100, nil, "", -1) }
